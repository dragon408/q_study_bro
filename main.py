...
def verify_user(message, username):
    ...
    if user_id:
        ...
        bot.send_message(message.chat.id, """
                    Привіт! Ось команди для бота ✍️
                    
                    /start – почати
                    /photo – надіслати фото з якого запишуться питання (по одному фото! )
                    /add – написати питання вручну
                    /random – отримати випадкове питання (воно видалиться зі списку)
                    /list – переглянути всі питання
                    /delete_all – видалити всі питання…
""")
    else:
